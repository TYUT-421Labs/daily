...
class VGG_Ocean_(nn.Module):
    def __init__(self):
        super(VGG_Ocean_, self).__init__()
        ...
        self.score_size = 17  # score size -> 25 -> 17
    ...
    def _IOULoss(self, pred, target, weight=None):
        ...
        pred_right = pred[:, 2]
        pred_bottom = pred[:, 3]
        # torch.nonzero(torch.lt(pred[:, 0], 0)).squeeze(1)
        ...
        # torch.sum(torch.eq(area_union, -1))  // torch.any(tensor == 3)
    ...
    def track(self, x):
        ...
        if self.align_head is not None:
            ...
            bbox_pred, cls_pred = self.connect_model(xf, self.zf)
