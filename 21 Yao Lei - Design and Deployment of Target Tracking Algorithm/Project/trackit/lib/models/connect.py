...
def plain_xcorr_depthwise(x, kernel):
    """
    cross correlation
    """
...
class AdaptiveConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = ops.DeformConv2d(in_channels, out_channels, 3, padding=1)
...
class MultiDW(nn.Module):
    """
    encode backbone feature
    """

    def __init__(self, in_channels=256):
        super(MultiDW, self).__init__()
        self.weight = nn.Parameter(torch.ones(3))

    def forward(self, z, x):
        # re11 = plain_xcorr_depthwise(x, z)
        # return re11

        z11, z12, z21 = z
        x11, x12, x21 = x

        re11 = plain_xcorr_depthwise(x11, z11)
        re12 = plain_xcorr_depthwise(x12, z12)
        re21 = plain_xcorr_depthwise(x21, z21)
        re = [re11, re12, re21]
...
class single_tower(nn.Module):
    ...
    def __init__(self, inchannels=128, outchannels=128, towernum=1):
        super(single_tower, self).__init__()
        tower = []
        cls_tower = []
        # encode backbone
        self.cls_encode = matrix(in_channels=inchannels, out_channels=outchannels)
        self.cls_dw = MultiDW(in_channels=inchannels)

        tower.append(nn.Conv2d(in_channels=inchannels, out_channels=outchannels, kernel_size=3, stride=1, padding=0,
                               dilation=1, groups=1, bias=False))
        tower.append(nn.BatchNorm2d(outchannels))
        tower.append(nn.ReLU())

        tower.append(nn.Conv2d(in_channels=outchannels, out_channels=outchannels, kernel_size=3, stride=1, padding=0,
                               dilation=1, groups=1, bias=False))
        tower.append(nn.BatchNorm2d(outchannels))
        tower.append(nn.ReLU())
        """
        tower.append(nn.Conv2d(in_channels=outchannels, out_channels=outchannels, kernel_size=3, stride=1, padding=1,
                               dilation=1, groups=1, bias=False))
        tower.append(nn.BatchNorm2d(outchannels))
        tower.append(nn.ReLU())
        ...
        # reg head
        self.bbox_pred = nn.Conv2d(outchannels, 4, kernel_size=3, stride=1, padding=0)
    ...
    def forward(self, search, kernal, update=None):
        ...
        cls_z, cls_x = self.cls_encode(kernal, search)  # [z11, z12, z13]
        # cls and reg DW
        cls_dw = 0.1 * self.cls_dw(cls_z, cls_x)
        # cls_dw = 0.1 * self.cls_dw(kernal, search)
        x_reg = self.bbox_tower(search)
        x = self.adjust * self.bbox_pred(x_reg) + self.bias
        x = torch.exp(x)
...
if __name__ == '__main__':
    search = torch.ones(8, 128, 23, 23).cuda()
    kernel = torch.ones(8, 128, 7, 7).cuda()
    net = single_tower().cuda()
    print((net(search, kernel))[0].shape)   # 位置信息
    print((net(search, kernel))[1].shape)   # 分类信息
